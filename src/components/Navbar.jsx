...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-shark-navy/95 backdrop-blur-sm shadow-lg py-4' : 'bg-transparent py-6'}`}>
      ...
      {isOpen && (
        <div className="md:hidden bg-shark-navy border-t border-shark-light">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} onClick={() => setIsOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-200 hover:text-white hover:bg-shark-light">
                {link.name}
              </a>
            ))}
            <a href="#contacto" onClick={() => setIsOpen(false)} className="block px-3 py-2 rounded-md text-base font-bold text-shark-navy bg-white hover:bg-gray-100 mt-4 text-center">
              Contacto
            </a>
          </div>
        </div>
